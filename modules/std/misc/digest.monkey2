...
Function MD5File:String(path:String,blocksize:int=1024)
	
	Local _file:=FileStream.Open(path,"r")
	If (_file)
		If _file.Length<blocksize blocksize=_file.Length
		Local _md5String:String
		Local _count:=_file.Length/blocksize
		Local _rest:=_file.Length Mod blocksize
		Local _buf:=_file.ReadAll()
		
		For Local a:=0 To _count-1
			_md5String+=_buf.PeekString(a*blocksize,blocksize)
		Next
			_md5String+=_buf.PeekString(_count*blocksize,_rest)
	
		Return MD5(_md5String)
		_file.Close()
	Else	
